...
import styles from "/public/styles/NftCreator.module.css";
...
export default function NftCreator({ contractAddress, abi }) {
  ...
  const generateMetadata = async () => {
    ...
    const { fileURL } = await fetch("/api/nft/pinFileToIpfs", {
      method: "POST",
      body: formData,
    }).then((res) => res.json());
    ...
    const { metadataURL } = await fetch("/api/nft/pinJsonToIpfs", {
      method: "POST",
